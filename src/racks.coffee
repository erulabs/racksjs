        ...
        unless resource._racksmeta.noResource
            ...
            resource.assume = (obj) =>
                ...
                if !obj.id? and !obj.name?
                    @log '[INFO] .assume() relies on .target() which in turn requires the object '
                    @log 'argument to have a .id or .name - please define one or the other - '
                    @log 'alternatively you can pass a string, in which case skinny will assume youre providing an id'
                    ...
                    target: () ->
                        ...
                        if @endpoints.length > 1
                            ...
                            @endpoints.forEach (endpoint) ->
                                ...
                                #console.log 'rack.network is', rack.network
                                #console.log 'and the result is:', target, 'available endpoints are', @endpoints
                        else
                            target = @endpoints[0]
